...
import {IcRoundAdd, IcRoundEdit} from '../icons.js'
...
export function UsersPage({dashboard}: UsersPageProps) {
  ...
  return (
    <Rail main>
      <RailHeader className={styles.UsersPage.header()}>
        <div className={styles.UsersPage.header.controls()}>
          <SearchField
      ...
      <RailBody>
        <div className={styles.UsersPage.content()}>
          {isLoading ? (
            ...
            <UsersTable
              users={filteredUsers}
              roleLabel={role => config.roles?.[role]?.label ?? role}
              dashboard={dashboard}
              onSaved={handleSaved}
            />
          )}
...
function userSearchText(
  ...
  return [
    ...
    user.email,
    user.sub,
    ...(user.roles ?? []).map(role => roles?.[role]?.label ?? role)
  ]
    .filter(Boolean)
    .join(' ')
    .toLowerCase()
