...
import {IcBaselinePause} from 'alinea/ui/icons/IcBaselinePause'
        ...
        >
          {
            <span className={styles.tree.item.icon()}>
              <Icon
                icon={
                  icon ??
                  (isFolder ? IcTwotoneFolder : IcTwotoneInsertDriveFile)
                }
              />
            </span>
          }
      ...
      <div {...tree.getContainerProps()} className={styles.tree()}>
        {loaded.map(item => {
          ...
          while (current && !isParentSelected) {
            if (!current.isFocused()) current = current.getParent()
